...
export function GrantsForWomenPage() {
  const [active, setActive] = useState<GrantCategory | "All">("All");

  const shown = useMemo(
    () =>
      active === "All"
        ? GRANTS
        : GRANTS.filter((g) => g.category === active),
    [active],
  );
    ...
    <main>
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <Button variant="lux" size="sm" asChild>
              <Link
                to="/auth"
                search={{ mode: "signup" }}
                onClick={() => trackStartFreeClick("grants_header")}
              >
                Start Free
              </Link>
            </Button>
      ...
      <section className="relative overflow-hidden px-5 pt-32 pb-12 md:px-10 md:pt-40 md:pb-16">
        ...
        <div className="relative mx-auto w-full max-w-3xl text-center">
          <p className="eyebrow eyebrow-blush">Free resource · Updated 2025</p>
          ...
          <p className="text-muted-foreground mx-auto mt-5 max-w-2xl text-[0.95rem] leading-relaxed md:mt-6 md:text-lg">
            16 real grants women entrepreneurs can apply for right now — from
            the $10,000 monthly Amber Grant to the $100,000 Cartier Women's
            Initiative. No email required. Browse by category, check your
            eligibility, and apply directly on each provider's site.
          </p>
      ...
      <Section className="py-10 md:py-12">
        <div className="mx-auto grid max-w-4xl grid-cols-2 gap-6 md:grid-cols-4">
          ...
          <div className="text-center">
            <p className="font-display text-gold text-3xl font-light md:text-4xl">$10K</p>
            <p className="text-muted-foreground mt-1 text-[0.7rem] tracking-[0.14em] uppercase">
              Avg. award
            </p>
          </div>
          <div className="text-center">
            <p className="font-display text-gold text-3xl font-light md:text-4xl">5</p>
            <p className="text-muted-foreground mt-1 text-[0.7rem] tracking-[0.14em] uppercase">
              Rolling deadlines
            </p>
          </div>
      ...
      <Section className="py-14 md:py-20">
        <div className="mx-auto max-w-3xl">
          <h2 className="font-display text-2xl leading-snug font-light md:text-4xl">
            How to qualify for a business grant
          </h2>
          <ol className="text-muted-foreground mt-7 space-y-5 text-sm leading-relaxed md:text-base">
            <li>
              <span className="text-foreground font-medium">1. Confirm you're eligible.</span>{" "}
              Most grants require you to be a woman-owned business in the U.S.
              Some ask for revenue, a minimum time in business, or a specific
              industry. Read the eligibility carefully before applying — it
              saves time.
            </li>
            <li>
            ...
            <li>
              ...
              Grants are competitive. Apply to rolling-deadline grants monthly
              (Amber Grant, IFundWomen, Hello Alice) and annual grants when they
              open. Track deadlines in a spreadsheet.
            </li>
            <li>
              <span className="text-foreground font-medium">4. Follow up and reapply.</span>{" "}
              If you don't win, reapply the next cycle. Many winners applied
              multiple times before being selected. Persistence matters more
              than perfection.
            </li>
          </ol>
        </div>
      ...
      <Section id="grants" className="pt-0 md:pt-0">
        <SectionHeading
          eyebrow="The grants"
          title="16 grants women can apply for now"
          lead="Filter by category. Every grant links to the official application page — no gate, no email required."
        />

        <div className="mt-10 flex flex-wrap justify-center gap-2 md:mt-12">
          {(["All", ...GRANT_CATEGORIES] as const).map((c) => (
            <button
              key={c}
              type="button"
              onClick={() => setActive(c)}
      ...
      <Section className="pt-0 md:pt-0">
        <div className="border-gold/30 bg-card/50 mx-auto max-w-2xl rounded-3xl border p-8 text-center md:p-12">
          ...
          <p className="text-muted-foreground mt-4 text-sm leading-relaxed">
            ...
            Empire Prompt Vault — including grant-writing prompts that help you
            draft your application, organize your business summary, and refine
          ...
          <Button variant="gold" size="xl" className="mt-7 w-full sm:w-auto" asChild>
            <Link
              to="/auth"
              search={{ mode: "signup" }}
              onClick={() => trackStartFreeClick("grants_cta")}
            >
              Start Free
            </Link>
          </Button>
      ...
      <Section id="faq" className="pt-0 md:pt-0">
        <div className="mx-auto max-w-3xl">
          <h2 className="font-display text-2xl leading-snug font-light md:text-4xl">
            Questions about grants for women
          </h2>
